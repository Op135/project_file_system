...
物料方案的料号不是发起方案评审的前置条件。评审通过后若新增/仅删除/调量缺少单一料号，或更改缺少改前、改后任一料号，单据停留在独立料号补充阶段；`ecn.material_code.edit` 控制处理人、首页待办和企业微信提醒。补充操作按方案打开时的料号快照检查并发冲突，最后一项补齐与执行清单生成在同一单据事务内完成，执行阶段拒绝继续修改。
