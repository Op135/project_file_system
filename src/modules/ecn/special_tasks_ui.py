...
def render_transfer_button(ecn_id: str, key: str, confirmation: dict, username: str, role: str, active: bool, refresh):
    with ui.column().classes("px-3 py-2 gap-1 items-center justify-center text-center self-stretch"):
        ui.label(str(confirmation.get("assignee") or "执行助理")).classes("text-xs")
        if active and confirmation.get("confirmed") is not True and can_execute_ecn_assistant_stage(role, username):
            ui.button(
        ...
        async def render():
            ...
            for key, item in assigned.items():
                ...
                with ui.row().classes("w-full items-center border-b py-2"):
                    ...
                    ui.label(
                        f"{item.get('assignee') or '执行助理'} · {'已完成' if item.get('confirmed') else '待确认'}"
                    )
                    ...
                    render_transfer_button(
                        ecn_id, key, item, username, role, active, lambda: (render.refresh(), refresh_list())
                    )
